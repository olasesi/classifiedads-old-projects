...
$page_title = 'Edit my shop slide products';					
$descript = 'Edit shop goods and services slide';		

?>
<?php
$query = mysqli_query($connect,"SELECT * FROM slider WHERE id_slider = '".mysqli_real_escape_string ($connect, $_GET['goods_no'])."' AND user_id_slider = '".$_SESSION['user_id']."'") or die(db_conn_error);
...
if (!isset($reg_errors_images)) {$reg_errors_images = array();}
 ...
 if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['uploadedit'])){
	...
	if (preg_match ('/^[A-Z0-9\'.\-, \(\)&]{3,20}$/i', $_POST['edit_goods_name'])) {		//only 20 characters are allowed to be inputted
 ...
 if (preg_match ('/^[0-9]{1,10}$/', trim($_POST['edit_price']))) {						//only numbers are allowed here. no decimals or commas
		$editprice = mysqli_real_escape_string ($connect,$_POST['edit_price']);
		} else {
		$reg_errors_edit['edit_price'] = 'Please enter correct price e.g 13000';
		}
		
	if (preg_match ('/0\d{10}/', trim($_POST['edit_number']))) {
		$editnumber = mysqli_real_escape_string ($connect,$_POST['edit_number']);		//match phone number
		} else {
...
//now to edit the product	
	if (empty($reg_errors_edit)){
			...
			mysqli_query($connect, "UPDATE slider SET slide_goods_name = '".$editgoodname."', slide_goods_price = '".$editprice."',slide_goods_cat = '".$editcategories."', slide_goods_number = '".$editnumber."', slide_local_area = '".$editarea."', slide_goods_desc = '".$editdescription_g."', slide_bonus_fee = '".$editbonus."' WHERE id_slider = '".mysqli_real_escape_string ($connect, $_GET['goods_no'])."' AND user_id_slider = '".$_SESSION['user_id']."'") or die(db_conn_error);
			if (mysqli_affected_rows($connect) == 1) {
			header("Location:".MYSHOPTWO."/full-details-slide.php?details=".mysqli_real_escape_string ($connect, $_GET['goods_no']));
			exit;
			}
} 
...
mysqli_query($connect, "UPDATE slider SET slider_image = '".$editnew_name."' WHERE id_slider = '".mysqli_real_escape_string ($connect, $_GET['goods_no'])."' AND user_id_slider = '".$_SESSION['user_id']."'") or die(db_conn_error);
			if (mysqli_affected_rows($connect) == 1) {
...
$all_about_goods = mysqli_query($connect, "SELECT * FROM slider WHERE id_slider = '".$_GET['goods_no']."'") or die(db_conn_error);

while ($row = mysqli_fetch_array($all_about_goods)) {
	$goods_name = $row['slide_goods_name'];
	$goods_price = $row['slide_goods_price'];
	$phone_number = $row['slide_goods_number'];
	$cat = $row['slide_goods_cat'];
	$goods_pic = $row['slider_image'];
	$about = $row['slide_goods_desc'];
	$bonus = $row['slide_bonus_fee'];
	$localarea = $row['slide_local_area'];
	}
